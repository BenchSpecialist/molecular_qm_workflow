...
class AIMNet2ASE(Calculator):
    implemented_properties = [
        'energy', 'forces', 'free_energy', 'charges', 'stress'
    ]

    def __init__(self,
                 base_calc: Union[AIMNet2Calculator, str] = 'aimnet2',
                 charge=0,
                 mult=1):
        super().__init__()
        ...
        if hasattr(base_calc, 'implemented_species'):
            self.implemented_species = base_calc.implemented_species.cpu(
            ).numpy()
        else:
    ...
    def set_atoms(self, atoms):
        if self.implemented_species is not None and not np.in1d(
                atoms.numbers, self.implemented_species).all():
            raise ValueError(
    ...
    def uptade_tensors(self):
        if self._t_numbers is None:
            self._t_numbers = torch.tensor(self.atoms.numbers,
                                           dtype=torch.int64,
                                           device=self.base_calc.device)
        if self._t_charge is None:
            self._t_charge = torch.tensor(self.charge,
                                          dtype=torch.float32,
                                          device=self.base_calc.device)
        if self._t_mult is None:
            self._t_mult = torch.tensor(self.mult,
                                        dtype=torch.float32,
                                        device=self.base_calc.device)
        if self._t_mol_idx is None:
            self.mol_idx = torch.zeros(len(self.atoms),
                                       dtype=torch.int64,
                                       device=self.base_calc.device)
    ...
    def calculate(self,
        ...
        results = self.base_calc(
            {
                'coord':
                torch.tensor(self.atoms.positions,
                             dtype=torch.float32,
                             device=self.base_calc.device),
                'numbers':
                self._t_numbers,
        ...
        for k, v in results.items():
            ...
            # Ensure energy is always a scalar for ASE compatibility
            if k == 'energy':
                numpy_val = float(
                    numpy_val.item() if numpy_val.ndim == 0 else (
                        numpy_val[0] if numpy_val.size > 0 else numpy_val))
            results[k] = numpy_val
