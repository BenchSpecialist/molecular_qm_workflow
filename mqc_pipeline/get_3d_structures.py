...
from .common import Molecule3DInfo

MAX_ATTEMPTS_EMBED = 100

def smiles_to_3d_structures_by_rdkit(smiles: str, max_attempts: float = MAX_ATTEMPTS_EMBED) -> Molecule3DInfo:
    """
    Convert a SMILES string to a 3D structure using RDKit.

    :param smiles: A SMILES string representing the molecule.
    :return: A Molecule3DInfo object containing the elements, 3D coordinates, and SMILES string.
    ...
    for attempt in range(max_attempts):
        ...
        if attempt == max_attempts - 1:
            raise ValueError(f"Failed to embed molecule for SMILES after {max_attempts} attempts: {smiles}")
    ...
    return Molecule3DInfo(elements=elements, xyz=xyz, smiles=smiles)
