...
class AEVSV(nn.Module):
    ...
    def _init_basis(self, rc, eta, nshifts, shifts, rmin, mod="_s"):
        self.register_parameter(
            "rc" + mod,
            nn.Parameter(torch.tensor(rc, dtype=torch.float),
                         requires_grad=False),
        )
        if eta is None:
        ...
        self.register_parameter(
            "eta" + mod,
            nn.Parameter(torch.tensor(eta, dtype=torch.float),
                         requires_grad=False),
        )
        if shifts is None:
        ...
        self.register_parameter("shifts" + mod,
                                nn.Parameter(shifts, requires_grad=False))
    ...
    def _calc_aev(self, r_ij: Tensor, d_ij: Tensor,
                  data: Dict[str, Tensor]) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        gs = ops.exp_expand(d_ij, self.shifts_s, self.eta_s) * fc_ij.unsqueeze(
            -1)  # (..., m, nshifts) * (..., m, 1) -> (..., m, shitfs)
        u_ij = r_ij / d_ij.unsqueeze(
            -1)  # (..., m, 3) / (..., m, 1) -> (..., m, 3)
        if self._dual_basis:
...
class ConvSV(nn.Module):
    ...
    def forward(self,
                a: Tensor,
                gs: Tensor,
                gv: Optional[Tensor] = None) -> Tensor:
        avf = []
        if self.d2features:
