...
def load_yaml(
        ...
        hyperpar: Optional[Dict[str, Any] | str] = None
) -> Dict[str, Any] | List:
    """
        ...
        if hyperpar:
            for d, k, v in _iter_rec_bottomup(config):
                if isinstance(v, str) and "{{" in v:
                    d[k] = Template(v).render(
                        **hyperpar)  # type: ignore[assignment, index]
    else:
    ...
    for d, k, v in _iter_rec_bottomup(config):  # type: ignore[arg-type]
        if isinstance(v, str) and any(
                v.endswith(x) for x in (".yml", ".yaml")):
...
def build_module(
        ...
        hyperpar: Union[str, Dict,
                        None] = None) -> Union[List, Dict, Callable]:
    """
