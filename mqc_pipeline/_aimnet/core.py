...
from .config import get_init_module, get_module
...
def MLP(n_in: int,
    ...
    if isinstance(activation_fn, str):
        activation_fn = get_init_module(activation_fn,
    ...
    for i in range(1, len(sizes)):
        ...
        if not (last_linear and i == len(sizes) - 1):
            ...
            if dropout > 0:  # Add dropout if specified
...
class Output(nn.Module):
    ...
    def __init__(self, mlp: Dict | nn.Module, n_in: int, n_out: int,
...
class Forces(nn.Module):
    ...
    def forward(self, data: Dict[str, Tensor]) -> Dict[str, Tensor]:
        ...
        y = data[self.y]
        g = torch.autograd.grad([y.sum()], [data[self.x]],
