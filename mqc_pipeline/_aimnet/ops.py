...
def calc_distances(data: Dict[str, Tensor],
                   suffix: str = "",
                   pad_value: float = 1.0) -> Tuple[Tensor, Tensor]:
    ...
    if f"shifts{suffix}" in data:
        ...
        nb_mode = nbops.get_nb_mode(data)
        if nb_mode == 2:
            shifts = torch.einsum("bnmd,bdh->bnmh", data[f"shifts{suffix}"],
                                  data["cell"])
        else:
...
def center_coordinates(coord: Tensor,
                       data: Dict[str, Tensor],
                       masses: Optional[Tensor] = None) -> Tensor:
    if masses is not None:
        masses = masses.unsqueeze(-1)
        center = nbops.mol_sum(coord * masses, data) / nbops.mol_sum(
            masses, data) / data["mol_sizes"].unsqueeze(-1)
    else:
...
def cosine_cutoff(d_ij: Tensor, rc: Union[float, Tensor]) -> Tensor:
    """
    Compute cosine cutoff function.

    Args:
        d_ij: Distance tensor
        rc: Cutoff radius (can be float or tensor)

    Returns:
        Cutoff function values
    """
    # Convert rc to tensor if it's a parameter
    if isinstance(rc, torch.nn.Parameter):
        rc = rc.data

    # Apply clamp without named arguments
    d_clamped = torch.clamp(d_ij, 1e-6, float(rc))
    fc = 0.5 * (torch.cos(d_clamped * (math.pi / rc)) + 1.0)
    return fc
...
def exp_cutoff(d: Tensor, rc: Tensor) -> Tensor:
    fc = torch.exp(
        -1.0 / (1.0 -
                (d / rc).clamp(0, 1.0 - 1e-6).pow(2))) / 0.36787944117144233
    return fc
...
def coulomb_matrix_dsf(d_ij: Tensor, Rc: float, alpha: float,
...
def coulomb_matrix_sf(q_j: Tensor, d_ij: Tensor, Rc: float,
                      data: Dict[str, Tensor]) -> Tensor:
    _c1 = 1.0 / d_ij
    _c2 = 1.0 / Rc
    _c3 = _c2 / Rc
...
def get_shifts_within_cutoff(cell: Tensor, cutoff: Tensor) -> Tensor:
    ...
    shifts = torch.cartesian_prod(
        torch.arange(-num_repeats[0], num_repeats[0] + 1,
                     device=device),  # type: ignore[attr-defined]
        torch.arange(-num_repeats[1], num_repeats[1] + 1,
                     device=device),  # type: ignore[attr-defined]
        torch.arange(-num_repeats[2], num_repeats[2] + 1,
                     device=device),  # type: ignore[attr-defined]
    ).to(torch.float)
    return shifts
...
def coulomb_matrix_ewald(coord: Tensor, cell: Tensor) -> Tensor:
    ...
    disps = disps_ij[None, :, :, :] + torch.matmul(shifts, cell)[:, None,
                                                                 None, :]
    distances_all = disps.norm(p=2,
                               dim=-1)  # (num_shifts, num_atoms, num_atoms)
    ...
    e_real_matrix_aug[within_cutoff] = torch.erfc(
        distances / (math.sqrt(2) * eta)) / distances
    ...
    e_recip_matrix = 4.0 * math.pi / volume * torch.sum(e_recip_matrix_aug,
                                                        dim=0)
    # self interaction
    device = coord.device
