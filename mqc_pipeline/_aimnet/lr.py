...
from . import constants, nbops, ops
...
class DispParam(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        if (ptfile is None and (ref_c6 is None or ref_alpha is None)
                or ptfile is not None and
            (ref_c6 is not None or ref_alpha is not None)):
            raise ValueError(
...
class D3TS(nn.Module):
    ...
    def forward(self, data: Dict[str, Tensor]) -> Dict[str, Tensor]:
        ...
        disp_param_i, disp_param_j = nbops.get_ij(disp_param,
                                                  data,
                                                  suffix="_lr")
        ...
        # TS combination rule
        c6ij = 2 * c6_i * c6_j / (c6_i * alpha_j / alpha_i +
                                  c6_j * alpha_i / alpha_j).clamp(min=1e-4)
        ...
        e_ij = c6ij * (self.s6 / (d_ij.pow(6) + r0ij.pow(6)) + self.s8 * rrij /
                       (d_ij.pow(8) + r0ij.pow(8)))
...
class DFTD3(nn.Module):
    ...
    def _calc_c6ij(self, data: Dict[str, Tensor]) -> Tensor:
        ...
        cn = cn_ij.sum(-1)
        cn = torch.clamp(cn,
                         max=self.cnmax[numbers]).unsqueeze(-1).unsqueeze(-1)
        ...
        l_ij = torch.exp(self.k3 * ((cn_i - cnref_i).pow(2) +
                                    (cn_j - cnref_j).pow(2)))
